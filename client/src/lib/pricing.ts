...
async function loadPricingData() {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/pricing-data`, {
      credentials: "include",
    });
    if (!response.ok) {
 ...
 */
export async function getApartmentPricingConfig(
  apartment: Apartment
): Promise<ApartmentPricingConfig> {
 ...
 */
async function getBasePriceForDate(
  apartment: Apartment,
  date: Date
): Promise<number> {
  await ensurePricingDataLoaded();
 ...
 */
async function getRuleSetForDate(
  apartment: Apartment,
  date: Date
 ...
 */
export async function calculateNightlyPrice(
  apartment: Apartment,
  date: Date
): Promise<number> {
 ...
 */
export async function getStayLimits(
  apartment: Apartment,
  startDate: Date
): Promise<{ minNights: number; maxNights: number }> {
  const ruleSet = await getRuleSetForDate(apartment, startDate);
  const config = await getApartmentPricingConfig(apartment);

  if (
    ruleSet &&
    ruleSet.minNights !== undefined &&
    ruleSet.maxNights !== undefined
  ) {
    return {
...
export async function calculateStayPrice(
  ...
  const stayLengthDiscountRate = getStayLengthDiscount(
    applicableDiscounts,
    totalNights
  );
  const stayLengthDiscountAmount = Math.round(
    subtotal * Math.abs(stayLengthDiscountRate)
  );
  ...
  if (stayLengthDiscountAmount > 0) {
    ...
    const appliedDiscount = applicableDiscounts.find(
      (d) => totalNights >= d.length && d.discount === stayLengthDiscountRate
    );
